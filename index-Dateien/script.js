...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "index-Dateien";
var g_showoutline = 1;
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			/*if ( IsFramesMode() )
				StatusPlay("Diese Präsentation wurde für eine ältere Browserversion optimiert. Sie verwenden eine neuere Version von Microsoft Internet Explorer oder Netscape Navigator und sollten die Präsentation optimieren, um den vollen Funktionsumfang Ihres Browsers dafür nutzen zu können."); */
			jpegArray( 1 );
		}
		else
			window.location.replace(  "index-Dateien/error.htm" );
	}
	else {
	...
	if ( IsFramesMode() && !isWebTV() )
		StatusPlay("Diese Präsentation enthält Informationen, die Ihr Browser u. U. nicht richtig anzeigen kann. Die Präsentation wurde für neuere Versionen von Microsoft Internet Explorer oder Netscape Navigator optimiert.");
	*/
	if ( isFLATFiles <= 0 ) {
	/*	if ( IsFramesMode() )
			window.alert("Diese Präsentation enthält Informationen, die Ihr Browser u. U. nicht richtig anzeigen kann. Die Präsentation wurde für neuere Versionen von Microsoft Internet Explorer oder Netscape Navigator optimiert." );
	*/
		window.location.replace(  "index-Dateien/error.htm" );
	}		
	/*
